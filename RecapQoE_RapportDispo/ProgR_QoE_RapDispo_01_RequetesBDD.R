...
dataHost <- read_csv2(paste0("C:/Users/siar_ycg8l6/ProgR/Centreon_RecapQoE_RapportDispo/",fichier_liste_machine,".csv")) %>% 
            arrange(nom_machine)
...
#                             inner join metrics m on v.id_metric = m.metric_id
#                             inner join index_data i on m.index_id = i.id
#                        WHERE (i.host_name LIKE ('%sel%') AND i.service_description LIKE ('%QoE-BRPP%')) AND
#                              ctime >= UNIX_TIMESTAMP('",date_deb,"') AND
#                              ctime <= UNIX_TIMESTAMP('",date_fin,"')
#                       "))
...
LastDate <- Result %>% select(service_description, moment) %>% 
     arrange(service_description,desc(moment)) %>% 
     mutate(chgt=ifelse(service_description!=lag(service_description) | row_number()==1,1,0)) %>% 
     filter(chgt==1)
...
# puis sauvegarde dans des fichiers Result.csv et Result_agreg.csv
