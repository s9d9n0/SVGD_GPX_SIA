...
options(scipen=100)
Requete <- Req_Data %>% mutate(POSIXct=ymd_hms(as_datetime(ctime))+dhours(decalage_heure)) %>% 
     select(-ctime) %>% 
     # converson en GB
     mutate(mesure=ifelse(unit_name=="B",round(value*9.313225746154785e-10,2),round(value,2))) %>% 
     select(-value)

df_quotas_data <- Requete %>% 
     ...
     mutate(quota=round(usage+free,1),
            conso=round(usage,0),
...
df_quotas_data_indic_quota <- df_quotas_data %>% 
     mutate(type="data",
...
df_quotas_data_indic_conso <- df_quotas_data %>% 
     mutate(type="data",
            an_mois_jour=str_sub(moment,1,8),
            heure=str_sub(moment,10,12),
            indicateur="conso") %>% 
...
df_quotas_data_indic_part <- df_quotas_data %>% 
     mutate(type="data",
            an_mois_jour=str_sub(moment,1,8),
            heure=str_sub(moment,10,12),
            indicateur="part") %>% 
...
ensemble <- rbind(df_quotas_data_indic_conso,
                  df_quotas_data_indic_quota,
                  df_quotas_data_indic_part) %>% 
            arrange(host_name,dc_zone,quartier,vv)

rm(df_quotas_data_indic_conso,
   df_quotas_data_indic_quota,
   df_quotas_data_indic_part)

     
# repertoire <- "C:/Users/siar_ycg8l6/ProgR/Analyse_Nas/Quotas_data/"
repertoire <- paste0(rep_gen,"Quotas_data/")
write.csv2(ensemble,
           paste0(repertoire,"df_quotas_data_",partie_an,partie_mois,partie_jour,".csv"),
           row.names=FALSE)
