...
rep_fich <- paste0("C:/Users/siar_ycg8l6/ProgR/HistoCentreon_User/Fichiers_date_",part_date2,"/")
setwd(rep_fich)
...
listing_centreonSIA <- listing %>%
     filter(str_detect(liste_fich,"CentreonSIA_User") & str_detect(liste_fich,"csv")) %>%
     arrange(liste_fich)
...
for (i in 1:nrow(listing_centreonSIA)){
     cat(paste0("numero ",i," ",listing_centreonSIA$liste_fich[i],"\n"))
     # concatenation des fichiers de type datareq
     if (i==1) {
          file.copy(paste0(rep_fich,listing_centreonSIA$liste_fich[i]),
                    paste0(rep_fich,"df_centreonSIA_user_jr_",part_date,lib_time,".csv"))
     } else {
          df_centreonSIA_jr <- read.csv2(paste0(rep_fich,"df_centreonSIA_user_jr_",part_date,lib_time,".csv"))
          df_copy <- read.csv2(paste0(rep_fich,listing_centreonSIA$liste_fich[i]))
          df_centreonSIA_jr <- rbind(df_centreonSIA_jr,df_copy)
          write.csv2(df_centreonSIA_jr,paste0(rep_fich,"df_centreonSIA_user_jr_",part_date,lib_time,".csv"),row.names=FALSE)
     }
     # insertion du fichier unitaire dans un .zip puis suppression
     #zip(paste0("df_centreonSIA_user_jr_",part_date,lib_time,".zip"),
     #    paste0(listing_centreonSIA$liste_fich[i]))
...
for (i in 1:nrow(listing_centreonSIA)){
     ...
     if (i==1) {
          ...
          zip_append(paste0("df_centreonSIA_user_jr_",part_date,lib_time,".zip"),
                     paste0(listing_centreonSIA$liste_fich[i]))
     }
}
...
# df_centreonSIA_jr <- read.csv2(paste0(rep_fich,"df_centreonSIA_user_jr_",part_date,lib_time,".csv"))
# 
# write.csv2(df_centreonSIA_jr,
#            paste0(rep_sas,"df_centreonSIA_user_jr_",part_date,lib_time,".csv"),
#            row.names=FALSE)
...
Agreg <- read.csv2(paste0(rep_fich,"df_centreonSIA_user_jr_",part_date,lib_time,".csv")) %>% 
...
# lib_time <- "_01h05"
# rep_fich <- "C:/Users/siar_ycg8l6/ProgrammesR_Gepex/HistoCentreon_User/Fichiers_date_20240107/"
# Agreg <- read.csv2(paste0(rep_fich,"df_centreonSIA_user_jr_",part_date,lib_time,".csv")) %>% 
#      arrange(contact_alias,session_id,POSIXct_moment_extract)
...
library(readODS)
write_ods(Agreg7,paste0(rep_fich,"SortieODS_centreonSIA_user_jr_",part_date,lib_time,".ods"), sheet="Visu")
